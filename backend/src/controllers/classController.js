...
async function getSlots(req, res, next) {
  try {
    ...
    const now = new Date();
    // Enriquecer con datos de disponibilidad y si el usuario actual reservó
    const result = slots.map(slot => ({
      ...slot,
      bookedCount: slot.bookings.length,
      available: slot.bookings.length < slot.maxStudents,
      userBooked: req.user ? slot.bookings.some(b => b.userId === req.user.id) : false,
      bookings: undefined,
    }));
    res.json(result);
  } catch (err) { next(err); }
...
async function getSlotDetail(req, res, next) {
  try {
    const slot = await prisma.classSlot.findUnique({
      ...
      include: { bookings: { where: { status: 'CONFIRMED' }, include: { user: { select: { id: true, name: true, email: true } } } } },
    });
    if (!slot) return res.status(404).json({ error: 'Clase no encontrada' });
    res.json(slot);
  } catch (err) { next(err); }
...
module.exports = { getSlots, bookSlot, cancelBooking, createSlot, deleteSlot, getSlotDetail };
